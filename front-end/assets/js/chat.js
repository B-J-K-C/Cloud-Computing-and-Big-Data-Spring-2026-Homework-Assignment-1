var checkout = {};
// Initialize the SDK client - Ensure apigClient.js is loaded before this script
  ...
  function insertMessage() {
    ...
    callChatbotApi(msg)
      .then((response) => {
        ...
        // Step 1: If the SDK returned a string, parse it into an object
        if (typeof data === 'string') {
            try { data = JSON.parse(data); } catch (e) { console.error("Parse error", e); }
        }

        // Step 2: If Lambda Proxy wrapped everything in a 'body' field, unpack it
        if (data && data.body) {
            var bodyData = (typeof data.body === 'string') ? JSON.parse(data.body) : data.body;
            data = bodyData;
        }
        
        console.log("Cleaned Data for UI:", data);
        // --- END UNPACKING ---
        ...
        if (data && data.messages && data.messages.length > 0) {
          ...
          for (var message of messages) {
            if (message.type === 'unstructured') {
              ...
              // Handle structured messages if your Lex bot sends them
              var html = '';
